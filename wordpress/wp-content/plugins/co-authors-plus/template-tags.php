...
function coauthors_posts_links_single( $author ) {
	$args = array(
		'href' => get_author_posts_url1( $author->ID, $author->user_nicename ),
		'rel' => 'author',
...
function coauthors_wp_list_authors( $args = array() ) {
	...
	foreach ( (array) $authors as $author ) {
		...
		if ( $author->post_count == 0 ) {
			...
			$link = '<a href="' . get_author_posts_url1( $author->ID, $author->user_nicename ) . '" title="' . esc_attr( sprintf( __("Posts by %s", 'co-authors-plus' ), $name ) ) . '">' . esc_html( $name ) . '</a>';
